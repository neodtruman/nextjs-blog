  ...
  return (
    <Fragment>
      <div className={styles['new-img-container']}>
        <span className={styles['btn-delete-image']}
          title="Remove"
          onClick={removeImageSelector}>X</span>
        <span className={styles['btn-copy-filename']}
          title="Select this as thumbnail"
          onClick={copyFileName}>S</span>
        <input className={styles['inp-image-file']}
          type="file"
          name="file"
          accept=".jpg, .jpeg, .png"
          onChange={uploadToClient}
          ref={inputFileRef} />
        {image && <img className={styles['new-img']}
          src={objectURL} alt=""
        />}
        {!image && <div
          className={styles['btn-select-image']}
          onClick={selectImageHandler}
        >Select Image</div>}
        <input type="text" readOnly
          className={styles['inp-copy']}
